...
void InitOpenGL()
{
	...
	gl_err = glGetError();
	if(gl_err != GL_NO_ERROR) printf("InitOpenGL: glEnable(GL_TEXTURE_2D) failed: 0x%x\n", gl_err);
	...
	gl_err = glGetError();
	if(gl_err != GL_NO_ERROR) printf("InitOpenGL: OpenGL initialization completed with error: 0x%x\n", gl_err);
#endif
}
...
void ResetOpenGLViewPort(int width, int height)
{
	...
	gl_err = glGetError();
	if(gl_err != GL_NO_ERROR) printf("ResetOpenGLViewPort: OpenGL error: 0x%x\n", gl_err);
#endif
}
