...
void ZPlayer::InitSDL()
{
	...
	if(use_opengl)
	{
		...
		if(!screen)
		{
			printf("InitSDL: SDL_SetVideoMode(OPENGL) failed, falling back to software mode\n");
			use_opengl = false;
	...
	//TTF
	TTF_Init();
	ttf_font = TTF_OpenFont(ResolveDataPath("assets/arial.ttf").c_str(),10);
	ttf_font_7 = TTF_OpenFont(ResolveDataPath("assets/arial.ttf").c_str(),7);
...
{
	const float fade_per_second = 128;
	const float splash_timeout = 30.0;
	static double last_time;
	static double start_time;
	static bool did_init = false;
	static bool graphics_was_loaded = false;

	if(!splash_screen.GetBaseSurface()) return;

	double cur_time = current_time();
	...
	//force graphics_loaded after timeout
	if(!graphics_loaded && (cur_time - start_time) >= splash_timeout)
	{
		printf("ZPlayer::DoSplash: graphics loading timeout after %.0f seconds, forcing proceed\n", splash_timeout);
		graphics_loaded = true;
	}

	//reset fade timer when graphics first becomes loaded
	if(graphics_loaded && !graphics_was_loaded)
	{
		graphics_was_loaded = true;
		last_time = cur_time;
		printf("ZPlayer::DoSplash: graphics loaded, starting fade from splash_fade=%.0f\n", splash_fade);
	}

	//if the game scene is already rendering, hide splash immediately
	if(graphics_loaded && zmap.Loaded() && splash_fade >= 5)
	{
		printf("ZPlayer::DoSplash: game scene ready, hiding splash\n");
		splash_fade = 0;
		splash_screen.SetAlpha(0);
	}
	...
	if(splash_fade >= 5)
	{
		...
		if(graphics_loaded)
		{
			splash_fade -= (float)(cur_time - last_time) * fade_per_second;
