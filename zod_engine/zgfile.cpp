...
string ASSETS_PATH = "assets"; // Default asset path
...
void ZGFile::LoadEntryList()
{
	...
	while(1)
	{
        ...
        printf("ZGFile::LoadEntryList: entries loaded:%zu\n", entry_list.size());
...
void ZGFile::AddFile(string filename, SDL_Surface *img)
{
	...
	{
		printf("ZGFile::AddFile: filename too long [size:%zu] for '%s'\n", filename.length(), filename.c_str());
		return;
	}

	string full_path = ASSETS_PATH + "/" + ZGFILE_NAME;
	fp = fopen(full_path.c_str(), "ab");
